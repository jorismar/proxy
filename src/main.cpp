					...
					if(json.length() > 0) {
						if(!json.compare("open")) {
							...
							if(findSession(sessions, id) < 0) {
								sessions.push_back(new Session(id, /*g_controller_ip*/"192.168.0.102", g_initial_udp_port++, g_initial_tcp_port++, g_site_path, g_dash_profile, g_dash_path + "/" + id, g_mpd_name, g_dash_on_the_fly, g_on_the_fly_buffer_size));
